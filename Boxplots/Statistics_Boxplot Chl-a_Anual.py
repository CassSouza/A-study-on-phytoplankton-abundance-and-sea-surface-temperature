...
df_stats = pd.DataFrame(columns=['Year', 'Area', 'Mean', 'Median', 'Minimum', 'Maximum', 'Q1', 'Q3', 'Lower Outliers', 'Upper Outliers'])

for area in df_chla['state'].unique():
    df = df_chla.loc[df_chla['state'] == area]
    ...
    ax, b_plot = df.boxplot(column='chlor_a', by='year', 
    ...
    plt.ylim(0, 6) #Para decidir os limites do plot (no exemplo, vai de 0 a 4.5)

    plt.title(f'{area}', loc='center', fontsize=18)
    plt.suptitle('')
    plt.xlabel('Mês', fontsize=14)
    plt.ylabel('Concentração de clorofila (mg/m3)', fontsize=14)
    ...
    for i in enumerate(categories):
        dict_ = {'Year': i[1],'Area': area, 'Mean': means[(i[0])], 'Median': medians[(i[0])], 'Minimum': minimums[(i[0])], 'Maximum': maximums[(i[0])], 'Q1': q1[(i[0])], 'Q3': q3[(i[0])], 'Lower Outliers': lower_outliers[(i[0])], 'Upper Outliers': upper_outliers[(i[0])]}
        df_dict = pd.DataFrame([dict_])
        df_stats = pd.concat([df_stats, df_dict], ignore_index=True)

df_stats.to_csv(f'{git_repo.working_tree_dir}\\Databases\\Stats_chla_Anual.csv')